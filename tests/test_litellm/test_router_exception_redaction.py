...
The Router appends internal config names (model_group, fallback_model_group,
fallback failure detail, deployment timeouts, context_window_fallbacks dict,
etc.) onto the message of the exception it re-raises. That message is then
surfaced to clients by ProxyException, leaking the proxy's internal wiring and,
when fallbacks are configured as inline dicts, provider credentials.

The flag defaults to False so none of that internal detail reaches clients.
Set it to True to opt into the historical debug behavior; even then, provider
credentials inside fallback configs are masked before being embedded in the
message.

These tests verify that with the flag OFF (default) the leak strings are
redacted from the raised exception's message, that with the flag ON the
topology strings reappear for debugging, and that a raw provider credential
never appears in the message regardless of the flag.
...
def _restore_expose_flag():
    """Save and restore the flag so each test controls it explicitly."""
