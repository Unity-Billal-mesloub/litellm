...
class TestMCPServerManager:
    ...
    def test_forwarded_modes_reject_client_credentials_config(self):
        from pydantic import ValidationError

        for mode in (MCPAuth.true_passthrough, MCPAuth.oauth_delegate):
    ...
    @pytest.mark.asyncio
    async def test_create_mcp_client_forwarded_modes_do_not_inject_server_credential(self):
        manager = MCPServerManager()
        server = MCPServer(
            server_id="tp-egress",
            name="tp",
        ...
        with (
            ...
            await manager._create_mcp_client(server=server, subject_token="upstream-token", cred_provider=MagicMock())
        mock_resolve.assert_not_awaited()
        assert mock_client_cls.call_args.kwargs["auth_value"] is None
