...
class TestMCPDelegateAuthToUpstream:
    ...
    async def test_true_passthrough_with_client_credentials_requires_admission(self):
        """A true_passthrough server that also declares oauth2_flow=client_credentials must NOT skip
        admission, or an anonymous caller could reach the upstream as LiteLLM's service account. The
        model validator normally rejects this combo, so ``model_construct`` builds it to exercise the
        ingress fail-closed guard directly."""
        from fastapi import HTTPException
        ...
        scope = {
            "type": "http",
            "method": "POST",
            "path": "/mcp/tp_m2m_server",
            "headers": [],
        }
        server = MCPServer.model_construct(
            server_id="tp-m2m",
            name="tp_m2m_server",
            transport="http",
            auth_type=MCPAuth.true_passthrough,
