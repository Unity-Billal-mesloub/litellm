...
def get_next_standardized_reset_time(
    ...
    if unit == "d":
        ...
        return _handle_day_reset(
            current_time, base_midnight, value * 7, reset_time_of_day
        )
    elif unit == "h":
        ...
        return _handle_month_reset(
            current_time, base_midnight, value, reset_time_of_day
        )
    else:
...
def _handle_day_reset(
    ...
    if value == 1:  # Daily reset at the configured time of day
        return _next_occurrence(
            base_midnight, reset_time_of_day, current_time, timedelta(days=1)
        )
        ...
        return _next_occurrence(
            upcoming_monday, reset_time_of_day, current_time, timedelta(days=7)
        )
        ...
        return _apply_time_of_day(
            base_midnight + timedelta(days=value), reset_time_of_day
        )
...
def _handle_month_reset(
    ...
    if candidate <= current_time:
        return _apply_time_of_day(
            _first_of_next_month(first_of_this_month), reset_time_of_day
        )
    return candidate
