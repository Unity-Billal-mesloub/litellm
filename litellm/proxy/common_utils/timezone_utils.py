...
def parse_budget_reset_time(raw: object) -> time:
    ...
    if not isinstance(raw, str):
        raise ValueError(
            f"Invalid budget_reset_time {raw!r}; must be a quoted 24-hour 'HH:MM' "
            'string, e.g. "12:00"'
        )
    for fmt in ("%H:%M", "%H:%M:%S"):
        try:
            ...
            continue
    raise ValueError(
        f"Invalid budget_reset_time {raw!r}; expected a 24-hour 'HH:MM' or "
        "'HH:MM:SS' string, e.g. \"12:00\""
    )
...
def get_budget_reset_settings() -> BudgetResetSettings:
    ...
    return BudgetResetSettings(
        ...
        reset_time_of_day=parse_budget_reset_time(
            getattr(litellm, "budget_reset_time", None)
        ),
    )


def compute_budget_reset_at(
    budget_duration: str, settings: BudgetResetSettings
) -> datetime:
