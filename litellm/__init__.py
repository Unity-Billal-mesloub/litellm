...
# Defaults to False so the proxy's internal model_name / fallback wiring is
# never surfaced to clients via ProxyException.message. When set to True, the
# Router appends internal config names (model_group, fallback model groups,
# deployment timeouts, fallback failure details) onto exception messages for
# debugging; provider credentials in fallback configs stay masked even then.
